...
temp.slp <- slp[which(rownames(slp) == "04/01/50") : which(rownames(slp) == "08/01/68"),] 
...
# and smooth
pc1.temp.sm <- zoo::rollmean(pc1.temp, 6, fill = NA, align = "right")
...
temp.dat <- temp.dat %>%
  ...
  dplyr::summarise(pc1.sm = mean(pc1.temp.sm))
...
ggplot(slp.wind.dat1, aes(pc1.sm, SE.wind.Oct.Apr)) +
  geom_point()


# second window - response = 1969:1988
  temp.slp <- slp[which(rownames(slp) == "04/01/68") : which(rownames(slp) == "08/01/88"),] 
  ...
  ggplot(slp.wind.dat2, aes(pc1.sm, SE.wind.Oct.Apr)) +
    geom_point()
  
  # ok...now the other two eras!
  # 1989 - 2008
  temp.slp <- slp[which(rownames(slp) == "04/01/88") : which(rownames(slp) == "08/01/08"),] 
  ...
  ggplot(slp.wind.dat3, aes(pc1.sm, SE.wind.Oct.Apr)) +
    geom_point()
  
  
  plot.dat <- rbind(slp.wind.dat1, slp.wind.dat2, slp.wind.dat3)
  
  ggplot(plot.dat, aes(pc1.sm, SE.wind.Oct.Apr, color = era)) +
    geom_point() +
    geom_smooth(method = "lm", se = F)
